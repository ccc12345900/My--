...
@torch.no_grad()
def run(weights='weights/best.pt',  # 权重文件地址 默认 weights/yolov5.pt
    ...
    return im0,re_label,len(xyxy)/4
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default='./mydata/images/test', help='file/dir/URL/glob, 0 for webcam')
...
def app_detect(conf):
    opt = parse_opt_app(conf)
    image_array,re_label,num = main(opt)
    print("ok")
    return image_array,re_label,num
...
def main(opt):
    ...
    # 执行run 开始推理
    image_array,re_label,num = run(**vars(opt))
    return image_array,re_label,num
